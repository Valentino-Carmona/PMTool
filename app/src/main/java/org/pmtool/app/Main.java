...
public class Main {
  ...
  private static void mostrarActividadConJerarquia(
      Actividad actividad, String prefix, String numeracion) {
  ...
  private static void mostrarProyectoFormateado(Proyecto proyecto) {
    ...
    System.out.println(
        "Fecha de fin estimada: " + formatearFecha(proyecto.getFechaFinPlanificada()));
  ...
  private static void proyectoEjemplo() {
    ...
    Actividad planificacion =
        gerenteProyecto.crearActividad(proyecto, "Planificación del proyecto", 0);
    Actividad definicionReq =
        gerenteProyecto.crearSubActividad(planificacion, "Definir requisitos", 3);
    ...
    Actividad implementacion =
        gerenteProyecto.crearSubActividad(desarrollo, "Implementar funcionalidad", 5);
    Actividad entorno =
        gerenteProyecto.crearSubActividad(desarrollo, "Preparar entorno de despliegue", 2);
    ...
    gerenteProyecto.configurarDependencia(
        disenoUI, definicionReq, 0); // Diseño depende de Definir requisitos
    gerenteProyecto.configurarDependencia(
        implementacion, disenoUI, 0); // Implementación depende de Diseño
    gerenteProyecto.configurarDependencia(
        entorno, definicionReq, 2); // Entorno depende de Definir requisitos con 2 días de retraso
    gerenteProyecto.configurarDependencia(despliegue, entorno, 0); // Despliegue depende de Entorno
    gerenteProyecto.configurarDependencia(
        capacitacion, despliegue, -1); // Capacitación depende de Despliegue con 1 día de adelanto
  ...
  private static void cocaColaChatbotEjemplo() {
    ...
    */
    ...
    // === ACTIVIDADES ===
    Actividad planificacion =
        gerenteProyecto.crearActividad(proyecto, "Planificación del Chatbot", 0);
    Actividad definirReq =
        gerenteProyecto.crearSubActividad(planificacion, "Definir Requisitos del Chatbot", 0);
    Actividad reunionStake =
        gerenteProyecto.crearSubActividad(definirReq, "Reunión con Stakeholders", 2);
    ...
    Actividad logicaChatbot =
        gerenteProyecto.crearSubActividad(backend, "Desarrollar Lógica del Chatbot", 4);

    Actividad frontend =
        gerenteProyecto.crearSubActividad(desarrollo, "Implementación Frontend", 0);
    Actividad disenoUI =
        gerenteProyecto.crearSubActividad(frontend, "Diseñar Interfaz de Usuario", 3);
    Actividad integrarBackend =
        gerenteProyecto.crearSubActividad(frontend, "Integrar con Backend", 2);
    ...
    Actividad prod =
        gerenteProyecto.crearSubActividad(despliegue, "Preparar Entorno de Producción", 2);
    Actividad lanzamiento =
        gerenteProyecto.crearSubActividad(despliegue, "Lanzamiento Controlado", 1);

    // === CONFIGURACIÓN DE DEPENDENCIAS ===
    gerenteProyecto.configurarDependencia(
        docReq, reunionStake, 1); // 1.1.2 depende de 1.1.1 con +1 día
    ...
    gerenteProyecto.configurarDependencia(
        integrarBackend, disenoUI, -1); // 2.2.2 depende de 2.2.1 con -1 día

    gerenteProyecto.configurarDependencia(
        backendTest, integrarBackend, 0); // 3.1.1 depende de 2.2.2
    gerenteProyecto.configurarDependencia(
        frontendTest, backendTest, 1); // 3.1.2 depende de 3.1.1 con +1 día
  ...
  private static void mlAgentsEjemplo() {
    ...
    */
    ...
    // === ACTIVIDADES ===
    Actividad planificacion =
        gerenteProyecto.crearActividad(proyecto, "Planificación de Agentes Inteligentes", 0);
    Actividad definicionModelos =
        gerenteProyecto.crearSubActividad(planificacion, "Definición de Modelos", 0);
    Actividad reunionCiencia =
        gerenteProyecto.crearSubActividad(
            definicionModelos, "Reunión con Equipo de Ciencia de Datos", 2);
    Actividad seleccionAlg =
        gerenteProyecto.crearSubActividad(definicionModelos, "Selección de Algoritmos", 3);
    Actividad docTecnica =
        gerenteProyecto.crearSubActividad(definicionModelos, "Documentación Técnica", 2);

    Actividad disenoArq =
        gerenteProyecto.crearSubActividad(planificacion, "Diseño Arquitectónico", 0);
    Actividad disenoPipeline =
        gerenteProyecto.crearSubActividad(disenoArq, "Diseño del Pipeline de Datos", 4);
    Actividad disenoApiInf =
        gerenteProyecto.crearSubActividad(disenoArq, "Diseño de la API de Inferencia", 3);
    Actividad disenoModuloMon =
        gerenteProyecto.crearSubActividad(disenoArq, "Diseño del Módulo de Monitoreo", 2);

    Actividad desarrollo = gerenteProyecto.crearActividad(proyecto, "Desarrollo de Agentes", 0);
    Actividad impModelo =
        gerenteProyecto.crearSubActividad(desarrollo, "Implementación del Modelo", 0);
    Actividad preprocDatos =
        gerenteProyecto.crearSubActividad(impModelo, "Preprocesamiento de Datos", 5);
    Actividad entrenamiento =
        gerenteProyecto.crearSubActividad(impModelo, "Entrenamiento del Modelo", 5);
    Actividad validacion = gerenteProyecto.crearSubActividad(impModelo, "Validación del Modelo", 3);
    Actividad optimizacion =
        gerenteProyecto.crearSubActividad(impModelo, "Optimización de Hiperparámetros", 4);

    Actividad impFrontend =
        gerenteProyecto.crearSubActividad(desarrollo, "Implementación del Frontend", 0);
    Actividad disenoInterfaz =
        gerenteProyecto.crearSubActividad(impFrontend, "Diseño de la Interfaz de Control", 3);
    Actividad desarrolloDashboard =
        gerenteProyecto.crearSubActividad(impFrontend, "Desarrollo del Dashboard", 4);
    Actividad integracionBackend =
        gerenteProyecto.crearSubActividad(impFrontend, "Integración con Backend", 2);
    Actividad pruebasUsabilidad =
        gerenteProyecto.crearSubActividad(impFrontend, "Pruebas de Usabilidad", 2);

    Actividad pruebas = gerenteProyecto.crearActividad(proyecto, "Pruebas y Validación", 0);
    Actividad pruebasRendimiento =
        gerenteProyecto.crearSubActividad(pruebas, "Pruebas de Rendimiento", 0);
    Actividad pruebasLatencia =
        gerenteProyecto.crearSubActividad(pruebasRendimiento, "Pruebas de Latencia", 3);
    Actividad pruebasEscalabilidad =
        gerenteProyecto.crearSubActividad(pruebasRendimiento, "Pruebas de Escalabilidad", 4);
    Actividad pruebasPrecision =
        gerenteProyecto.crearSubActividad(pruebasRendimiento, "Pruebas de Precisión", 2);
    Actividad pruebasSeguridad =
        gerenteProyecto.crearSubActividad(pruebas, "Pruebas de Seguridad", 0);
    Actividad auditoriaCodigo =
        gerenteProyecto.crearSubActividad(pruebasSeguridad, "Auditoría de Código", 3);
    Actividad pruebasInyeccion =
        gerenteProyecto.crearSubActividad(pruebasSeguridad, "Pruebas de Inyección de Datos", 2);

    Actividad despliegue = gerenteProyecto.crearActividad(proyecto, "Despliegue en Producción", 0);
    Actividad prepEntorno =
        gerenteProyecto.crearSubActividad(despliegue, "Preparación de Entorno", 0);
    Actividad configServCloud =
        gerenteProyecto.crearSubActividad(prepEntorno, "Configuración de Servidores Cloud", 3);
    Actividad despliegueModelo =
        gerenteProyecto.crearSubActividad(prepEntorno, "Despliegue del Modelo", 2);
    Actividad despliegueFrontend =
        gerenteProyecto.crearSubActividad(prepEntorno, "Despliegue del Frontend", 1);

    Actividad lanzamiento = gerenteProyecto.crearSubActividad(despliegue, "Lanzamiento", 0);
    Actividad lanzamientoBeta =
        gerenteProyecto.crearSubActividad(lanzamiento, "Lanzamiento en Fase Beta", 2);
    Actividad monitoreoInicial =
        gerenteProyecto.crearSubActividad(lanzamiento, "Monitoreo Inicial", 3);
    Actividad lanzamientoGlobal =
        gerenteProyecto.crearSubActividad(lanzamiento, "Lanzamiento Global", 1);

    // === CONFIGURACIÓN DE DEPENDENCIAS ===
    gerenteProyecto.configurarDependencia(
        seleccionAlg, reunionCiencia, 1); // 1.1.2 depende de 1.1.1 con +1 día
    ...
    gerenteProyecto.configurarDependencia(
        disenoApiInf, disenoPipeline, -1); // 1.2.2 depende de 1.2.1 con -1 día
    gerenteProyecto.configurarDependencia(
        disenoModuloMon, disenoApiInf, 2); // 1.2.3 depende de 1.2.2 con +2 días

    gerenteProyecto.configurarDependencia(
        preprocDatos, disenoPipeline, 0); // 2.1.1 depende de 1.2.1
    gerenteProyecto.configurarDependencia(entrenamiento, preprocDatos, 0); // 2.1.2 depende de 2.1.1
    gerenteProyecto.configurarDependencia(
        validacion, entrenamiento, -1); // 2.1.3 depende de 2.1.2 con -1 día
    gerenteProyecto.configurarDependencia(
        optimizacion, validacion, 1); // 2.1.4 depende de 2.1.3 con +1 día

    gerenteProyecto.configurarDependencia(
        disenoInterfaz, disenoApiInf, 0); // 2.2.1 depende de 1.2.2
    gerenteProyecto.configurarDependencia(
        desarrolloDashboard, disenoInterfaz, -1); // 2.2.2 depende de 2.2.1 con -1 día
    gerenteProyecto.configurarDependencia(
        integracionBackend, desarrolloDashboard, 1); // 2.2.3 depende de 2.2.2 con +1 día
    gerenteProyecto.configurarDependencia(
        pruebasUsabilidad, integracionBackend, 0); // 2.2.4 depende de 2.2.3

    gerenteProyecto.configurarDependencia(
        pruebasLatencia, pruebasUsabilidad, 0); // 3.1.1 depende de 2.2.4
    gerenteProyecto.configurarDependencia(
        pruebasEscalabilidad, pruebasLatencia, 1); // 3.1.2 depende de 3.1.1 con +1 día
    gerenteProyecto.configurarDependencia(
        pruebasPrecision, pruebasEscalabilidad, -2); // 3.1.3 depende de 3.1.2 con -2 días

    gerenteProyecto.configurarDependencia(
        auditoriaCodigo, optimizacion, 0); // 3.2.1 depende de 2.1.4
    gerenteProyecto.configurarDependencia(
        pruebasInyeccion, auditoriaCodigo, 0); // 3.2.2 depende de 3.2.1

    gerenteProyecto.configurarDependencia(
        configServCloud, pruebasPrecision, 0); // 4.1.1 depende de 3.1.3
    gerenteProyecto.configurarDependencia(
        despliegueModelo, configServCloud, -1); // 4.1.2 depende de 4.1.1 con -1 día
    gerenteProyecto.configurarDependencia(
        despliegueFrontend, despliegueModelo, 1); // 4.1.3 depende de 4.1.2 con +1 día

    gerenteProyecto.configurarDependencia(
        lanzamientoBeta, despliegueFrontend, 0); // 4.2.1 depende de 4.1.3
    gerenteProyecto.configurarDependencia(
        monitoreoInicial, lanzamientoBeta, -1); // 4.2.2 depende de 4.2.1 con -1 día
    gerenteProyecto.configurarDependencia(
        lanzamientoGlobal, monitoreoInicial, 2); // 4.2.3 depende de 4.2.2 con +2 días
