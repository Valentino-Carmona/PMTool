...
public class PlanificarActividadesSteps {
  ...
  @When(
      "el Gerente de Proyecto define una nueva actividad con nombre {string} y duración {int} días")
  public void elGerenteDeProyectoDefineUnaNuevaActividadConNombreYDuracionDias(
    ...
    try {
      if (proyecto.getEstado() == Proyecto.EstadoProyecto.FINALIZADO) {
        throw new IllegalStateException(
            "No se pueden agregar actividades a un proyecto finalizado");
      }
  ...
  @Then(
      "se informa un mensaje indicando que no se pueden agregar actividades a proyectos finalizados")
  public void seInformaUnMensajeIndicandoQueNoSePuedenAgregarActividadesAProyectosFinalizados() {
    assertNotNull(excepcion);
    assertEquals(
        "No se pueden agregar actividades a un proyecto finalizado", excepcion.getMessage());
  }
  ...
  public void laFechaDeFinDelProyectoEsLaMaximaFechaDeFinDeLasActividades() {
    LocalDate maxFechaFin =
        proyecto.getActividades().stream()
